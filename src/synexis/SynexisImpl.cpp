...
int SynexisImpl::addTask(const std::string &prompt, const SamplingParams &sampling_params) {
    const bool hasMtmd = mtmd_context != nullptr;
    auto slot = findEmptySlot();
    if (!slot) {
        //No Free Slots, Try Later
        return -1;
    }
    if (hasMtmd) {
...
std::string SynexisImpl::get_result(int task_id) {
...
std::string SynexisImpl::tokenToPiece(llama_token token, bool special) {
    const llama_vocab *vocab = llama_model_get_vocab(model);

    // Try with a reasonably small buffer on the stack first
    char buf[64]; // fast path for most tokens
...
void SynexisImpl::run() {
    ...
    workerThread = std::thread(std::move(runner));
}
...
SynexisSlot *SynexisImpl::findEmptySlot() const {
    for (auto &slot: slots) {
